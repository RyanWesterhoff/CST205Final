  ...
  while(condition):
    ...
    if mainResponse == "HELP":
      ...
      if rooms[curRN][1].count(subR) > 0:
        examine(inventory, subR, curRN, files)
      else:
...
def examine(inventory, itemName, roomNumber, files):
...
def use(inventory, roomNumber, files):
